	...
	async init(): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			request.onupgradeneeded = (event) => {
				...
				// Create profiles store
				if (!db.objectStoreNames.contains(STORES.REQUEST_LOG)) {
					const requestLog = db.createObjectStore(STORES.REQUEST_LOG, {
						keyPath: "id",
					});
					requestLog.createIndex("by_createdAt", "createdAt");
				}
